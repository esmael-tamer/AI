...
const typingWordsEn = ["Fashion Store", "Electronics Shop", "Restaurant Menu", "Beauty Brand", "Bookstore"]
const typingWordsAr = ["متجر أزياء", "متجر إلكترونيات", "قائمة مطعم", "علامة تجميل", "مكتبة إلكترونية"]

export default function MTHero() {
  const { t, isAr } = useLang()
  const typingWords = isAr ? typingWordsAr : typingWordsEn
  ...
  const stats = [
    { icon: Store, value: "2,500+", label: t("Stores Created", "متجر تم إنشاؤه") },
    { icon: TrendingUp, value: "$12M+", label: t("Revenue Generated", "إيرادات محققة") },
    { icon: Star, value: "98%", label: t("Satisfaction Rate", "نسبة الرضا") },
  ]
    ...
    <section className="relative min-h-screen flex items-center justify-center px-4 pt-28 pb-20 bg-[#0a0a0a] overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto text-center relative">
        ...
        <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black tracking-tighter text-white uppercase leading-[0.9]">
          {isAr ? (
            <>
              <span className="block">تجارة إلكترونية</span>
              ...
              <span className="block">للعلامات التجارية</span>
            </>
          ) : (
            <>
        ...
        <p className="mt-4 text-base text-white/30 max-w-xl mx-auto leading-relaxed">
          {t(
            "Our AI assistant guides you through creating a professional online store. No coding required. Just answer a few questions and watch your store come to life.",
            "مساعدنا الذكي يرشدك لإنشاء متجر إلكتروني احترافي. بدون برمجة. فقط أجب عن بعض الأسئلة وشاهد متجرك يتحول لحقيقة."
          )}
        </p>
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          >
            {t("Start Building Free", "ابدأ البناء مجاناً")}
